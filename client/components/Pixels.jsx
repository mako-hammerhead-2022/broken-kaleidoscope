  ...
  const [style, setStyle ] = useState({ 
    ...
    //added onContextMenu function for right click to turn squares black
     const contextMenu = (evt) => {
      evt.preventDefault()
      setStyle({height: '13px', width: '3px', backgroundColor: 'black' })
     }


    //added onMouseEnter function that when you hover over squares they turn green 
    const mouseHandler = () => {
      setStyle({height: '3px', width: '3px', backgroundColor: 'green'})
    }


  //added onclick handler function so when you click they switch colours
  const clickHandler = () => {
    setStyle({height: '3px', width: '3px', backgroundColor: `#${Math.floor(Math.random() * 0x1000000).toString(16).padStart(6, 0)}` 
  })
  }
